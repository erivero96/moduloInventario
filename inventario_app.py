...
class InventarioApp:
    ...
    def build_registro_producto(self):
        ...
        self.precio_entry = self._crear_campo(frame, "Precio", 4, tipo="float")
        self.categoria_entry = self._crear_campo(frame, "Categoría ID", 5, tipo="int")
        self.stock_entry = self._crear_campo(frame, "Stock", 6, tipo="int")
    ...
    def build_actualizar_stock(self):
        ...
        self.nuevo_stock = self._crear_campo(frame, "Nuevo stock", 2, tipo="int")
    ...
    def build_listar_productos(self):
        ...
        if not productos:
            ...
            for p in productos:
                ttk.Label(scrollable_frame, text=str(p), font=("Segoe UI", 10)).pack(anchor="w", padx=10)
    ...
    def _crear_campo(self, frame, texto, fila, tipo=None):
        ...
        if tipo == "int":
            ...
            entry.bind("<FocusOut>", validate_int_input)
        elif tipo == "float":
            def validate_float_input(event):
                value = entry.get()
                try:
                    float(value)
                except ValueError:
                    entry.delete(0, tk.END)
            entry.bind("<FocusOut>", validate_float_input)
